...
def main():
    ...
    while True:
        ...
        num_sents = input("Enter the number of sentences for summary: ")
...
def remove_stop_words(speech):
    ...
    speech_no_stop = ''
    for word in nltk.word_tokenize(speech):
        if word.lower() not in stop_words:
            speech_no_stop += word + ' '
    
    return speech_no_stop

def get_word_freq(speech_no_stop):
    word_freq = nltk.FreqDist(nltk.word_tokenize(speech_no_stop.lower()))
    return word_freq
...
def score_sentences(speech, word_freq, max_words):
    ...
    for sent in sentences:
        ...
        if sent_word_count <= int(max_words):
            ...
            sent_scores[sent] /= sent_word_count
    return sent_scores
